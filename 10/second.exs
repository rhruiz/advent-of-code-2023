...
sim = fn
  ...
  tiles, {x, y}, direction, and_then ->
    tile = Grid.get(new_grid, {x, y})
    exit = tile.(direction)
    ...
    tiles =
      if tile in [&Pipe.l/1, &Pipe.j/1, &Pipe.seven/1, &Pipe.f/1] do
        [{x, y} | tiles]
      else
        tiles
      end

    and_then.(tiles, new_position, delta_to_entrance[{dx, dy}], and_then)
end

vertices = sim.([{xs, ys}], {xs + dx, ys + dy}, dir, sim)
 |> IO.inspect
length = length(vertices) + 1
